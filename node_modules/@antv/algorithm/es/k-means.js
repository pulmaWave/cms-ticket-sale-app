...
var kMeans = function kMeans(originNodes, k, involvedKeys, uninvolvedKeys, propertyKey) {
  ...
  var nodes = clone(originNodes); // 所有节点属性集合

  var properties = getAllProperties(nodes, propertyKey); // 所有节点属性one-hot特征向量集合s

  var allPropertiesWeight = oneHot(properties, involvedKeys, uninvolvedKeys); // 记录节点的原始index，与allPropertiesWeight对应
      ...
      for (var m = 0; m < nodes.length; m++) {
        if (!centroidIndexList.includes(m)) {
          ...
          for (var j = 0; j < centroids.length; j++) {
            // 求节点到质心的欧式距离
            var euclideanDistance = new Vector(allPropertiesWeight[nodes[m].originIndex]).euclideanDistance(new Vector(centroids[j]));
  ...
  while (true) {
    for (var i = 0; i < nodes.length; i++) {
      ...
      if (!(iterations === 0 && centroidIndexList.includes(i))) {
        for (var j = 0; j < centroids.length; j++) {
          // 求节点到质心的欧式距离
          var euclideanDistance = new Vector(allPropertiesWeight[i]).euclideanDistance(new Vector(centroids[j])); // 记录节点最近的质心的索引
